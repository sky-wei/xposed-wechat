...
public class ItemMenu extends FrameLayout {
    ...
    public ItemMenu(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            ...
            switch (attr) {
                case R.styleable.ItemMenu_sky_itemMenuName:
                    setName(a.getString(attr));
                    break;
                case R.styleable.ItemMenu_sky_itemMenuDesc:
                    setDesc(a.getString(attr));
                    break;
                case R.styleable.ItemMenu_sky_itemMenuLine:
                    setShowLine(a.getBoolean(attr, true));
                    break;
                case R.styleable.ItemMenu_sky_itemMenuTextSize:
                    setTextSize(a.getDimensionPixelSize(attr, 14));
                    break;
                case R.styleable.ItemMenu_sky_itemMenuShowMore:
